...
// はさみの音は、毛刈りの動きに合わせて、1 回ずつ鳴らす（Game::shearCues）。以前は、4 回のジョキを 1 つの音として
// 動きの開始と同時に鳴らしていて、約 0.56 秒画面が止まるうえ、はさみが出る前に鳴り終わっていた。
void Sound::joki() {
    // 「ジョキッ」：刈り終わり。少し長く、低めの帯域にして、刈り切った感じにする。
    noise(110, 600, 1400);
}

void Sound::snip() {
    // 「ジョキ」：刃が閉じる音。ノイズの短いバースト。動きの邪魔にならないよう約 35ms。
    noise(35, 1000, 1900);
}

void Sound::rub() {
    // 「ゴシッ」：角が木にこすれる音。ジョキより低い帯域の、短いバースト。
    noise(40, 500, 1100);
}
